...
class DemagField(object):
    ...
    def _init_N_component(self, perm, func_near, func_far):
        # dipole far-field
        shape = [1 if i==1 else 2*i-1 for i in self._mesh.n]
        ij = [torch.fft.fftfreq(n,1/n, dtype=torch.float64) for n in shape]
    ...
    def h(self, t, m):
        ...
        with Timer("ifft"):
            hx = torch.fft.irfftn(hx, dim = [i for i in range(3) if self._mesh.n[i] > 1], s = [n if n==1 else 2*n-1 for n in self._mesh.n if n > 1])
            hy = torch.fft.irfftn(hy, dim = [i for i in range(3) if self._mesh.n[i] > 1], s = [n if n==1 else 2*n-1 for n in self._mesh.n if n > 1])
            hz = torch.fft.irfftn(hz, dim = [i for i in range(3) if self._mesh.n[i] > 1], s = [n if n==1 else 2*n-1 for n in self._mesh.n if n > 1])
