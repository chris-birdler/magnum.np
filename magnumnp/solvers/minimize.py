...
class Minimizer(object):
    ...
    def minimize(self, rtol = 1e-4, dt = 1e-4, maxiter = 1000, method = 'dopri5', options = {'first_step':1e-7}):
        ...
        for i in range(maxiter):
            ...
            m = odeint(lambda t, m: self._dm(t, m), m, self._state.DoubleTensor([t, t + dt]), method=method, options=options)[1] # TODO: reuse Solver object?
            t += dt
    ...
    def minimize_event(self, tol = 1e-2, T = 10e-4, dt = 1e-4, maxiter = 1000, method = 'dopri5', options = {'first_step':1e-7}):
        ...
        t, m_opt = odeint(lambda t, m: self._dm(t, m), m, self._state.DoubleTensor([t, t+T]), method=method, options=options, event_fn = event_fn)
