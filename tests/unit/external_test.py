...
def test_different_tensors():
    ...
    torch.testing.assert_close(avg(external.h(state)), state.Tensor([1,0,0]))
    ...
    state.t = 0.
    torch.testing.assert_close(avg(external.h(state)), state.Tensor([0,0,0]), atol=1e-15, rtol=1e-15)
    state.t = 1.
