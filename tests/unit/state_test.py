...
def test_average_nonequi():
    ...
    torch.testing.assert_close(avg(state.m, state.cell_volumes), torch.tensor([1.,0.,0.]), atol=1e-15, rtol=1e-15)
    torch.testing.assert_close(avg(state.material["A"], state.cell_volumes), torch.tensor([1.]), atol=1e-15, rtol=1e-15)
    torch.testing.assert_close(avg(z, state.cell_volumes), torch.tensor(5e-9), atol=1e-15, rtol=1e-15)
    torch.testing.assert_close(avg(state.m[:,:,:2,:], state.cell_volumes[:,:,:2]), torch.tensor([1.,0.,0.]), atol=1e-15, rtol=1e-15)
